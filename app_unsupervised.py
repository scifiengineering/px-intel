...
st.markdown("**Landscape Mapping of Hospital Feedback** | LDA + K-Means + RoBERTa Audit")
...
@st.cache_resource
def run_causal_reasoning(cluster_lda_features, cluster_vocabularies, cluster_sentiments):
...
def main():
    ...
    tab_landscape, tab_audit, tab_causal, tab_data = st.tabs(
        ["🗺️ Landscape", "📊 Cluster Audit", "⚡ Causal Analysis", "📋 Data Export"]
    )
    ...
    with tab_landscape:
        ...
        with col1:
            ...
            st.markdown("Each point is a feedback entry. Colors indicate clusters. Hover for details.")
            ...
            for cluster_id in np.unique(cluster_assignments):
                ...
                sentiment_dist = audit_engine.cluster_sentiment_results[cluster_id]['sentiment_distribution']
                zone = audit_engine.cluster_zones[cluster_id]['zone_type']
                
                # Color by zone
                color_map = {'RED_ZONE': '#FF6B6B', 'GREEN_ZONE': '#51CF66', 'NEUTRAL_ZONE': '#FFD93D'}
                ...
                fig.add_trace(go.Scatter(
                    ...
                    text=[f"Cluster {cluster_id}<br>Sentiment: {sentiment_dist['NEGATIVE']:.1%} negative" 
    ...
    with tab_causal:
        st.markdown("### Causal Reasoning Results")
        st.markdown(causal_engine.get_summary())
        
        # Cluster selector for causal details
        selected_causal_cluster = st.selectbox(
            "Select Cluster for Causal Details",
            sorted(causal_engine.cascade_predictions.keys()),
            format_func=lambda x: f"Cluster {x}"
        )
        
        if selected_causal_cluster in causal_engine.cascade_predictions:
            cascades = causal_engine.cascade_predictions[selected_causal_cluster]
            
            st.markdown(f"#### Cascade Predictions for Cluster {selected_causal_cluster}")
    ...
    with tab_data:
        ...
        sentiment_data = pd.DataFrame([
            {
                'cluster_id': cid,
                'sentiment_dominant': audit_engine.cluster_sentiment_results[cid]['dominant_sentiment'],
                'sentiment_density': audit_engine.cluster_sentiment_results[cid]['sentiment_density'],
        ...
        with col1:
            ...
            st.download_button(
                ...
                data=open("clustering_results.pkl", "rb").read() if Path("clustering_results.pkl").exists() else b"",
