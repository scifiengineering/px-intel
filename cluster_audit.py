...
class ClusterAuditEngine:
    ...
    def load_sentiment_model(
        ...
        if self.sentiment_pipeline is None:
            ...
            self.sentiment_pipeline = pipeline(
                "sentiment-analysis", model=model_name, device=self.model_device
            )
    ...
    def identify_cluster_zones(self) -> None:
        ...
        Zone Rules:
        - RED ZONE: Dominant sentiment is NEGATIVE
        - GREEN ZONE: Dominant sentiment is POSITIVE
        - NEUTRAL ZONE: Dominant sentiment is NEUTRAL or unavailable
        """
        self.cluster_zones = {}
    ...
    def get_red_zones(self) -> List[int]:
        ...
        return [
            cid
            for cid, zone in self.cluster_zones.items()
            if zone["zone_type"] == "RED_ZONE"
        ]
    ...
    def get_neutral_zones(self) -> List[int]:
        ...
        return [
            cid
            for cid, zone in self.cluster_zones.items()
            if zone["zone_type"] == "NEUTRAL_ZONE"
        ]
    ...
    def extract_cluster_vocabulary(
        self, cluster_id: int, n_keywords: int = 10, top_n_texts: int = 100
    ...
    def generate_cluster_audit_report(self, cluster_id: int) -> str:
        ...
        themes = [self._clean_phrase(keyword) for keyword, _ in vocabulary[:5]]
...
🧭 FEEDBACK PROFILE
    Main Tone: {sentiment_result['dominant_sentiment']}
    Tone Mix:
    ...
    Concentration: {sentiment_result['sentiment_density']:.3f}
    Confidence: {sentiment_result['avg_confidence']:.3f}
    Feedback Items Reviewed: {sentiment_result['num_texts']}

🎯 PRIORITY LEVEL
    Cluster Label: {zone_info.get('zone_type', 'UNKNOWN')}
    Priority Score: {zone_info.get('intensity_score', 0):.3f}
    Staff Attention: {zone_info.get('distress_level', 'Unknown')}
...
"""
        ...
        if themes:
            ...
            report += "  (No clear themes extracted)\n"

        report += "\n📝 WHAT THIS MEANS FOR MANAGEMENT\n"
        if zone_info.get("zone_type") == "RED_ZONE":
            report += f"  🚨 This cluster contains repeated friction signals. {sentiment_result['sentiment_distribution']['NEGATIVE']:.1%} "
            report += "of the feedback is negative.\n"
            report += "  Priority: Review the themes above and decide what operational step to change first.\n"
        elif zone_info.get("zone_type") == "GREEN_ZONE":
            report += f"  ✨ This cluster is working well. {sentiment_result['sentiment_distribution']['POSITIVE']:.1%} "
            report += "of the feedback is positive.\n"
            report += "  Priority: Keep the current approach and consider using it as a reference for other areas.\n"
        else:
            report += "  ⚖️  This cluster is mixed or balanced.\n"
            report += "  Priority: Watch the themes above and see whether they become more positive or negative over time.\n"
    ...
    def audit(
        self, texts: List[str], cluster_assignments: np.ndarray, n_keywords: int = 10
    ) -> None:
        """
