...
class CausalReasoningEngine:
    ...
    def _clean_phrase(text: str) -> str:
        """Convert model-generated keyword fragments into plain language."""
        return text.replace("_", " ").replace("-", " ").strip()

    def _get_theme_phrases(self, cluster_id: int, top_n: int = 5) -> List[str]:
        """Return the most visible themes for a cluster in plain language."""
        vocabulary = self.cluster_vocabularies.get(cluster_id, [])
        return [self._clean_phrase(keyword) for keyword, _ in vocabulary[:top_n]]

    # ========================================================================
    # Model Loading
    # ========================================================================

    def load_nli_model(self, model_name: str = "facebook/bart-large-mnli"):
        """
        ...
        if self.deberta_model is None:
            ...
            self.deberta_model = pipeline(
                "zero-shot-classification", model=model_name, device=self.model_device
            )
    ...
    def validate_causal_linkage(
        ...
        # Use explicit zero-shot labels instead of relying on entailment label ids.
        text = f"Patients reported {keyword}."
        positive_label = f"direct cause of {sentiment_label.lower()} patient experience"
        negative_label = (
            f"not a direct cause of {sentiment_label.lower()} patient experience"
        )
        ...
        try:
            result = self.deberta_model(
                ...
                hypothesis_template="This feedback is {}.",
                multi_label=False,
                truncation=True,
                max_length=512,
            )

            # Extract the best-scoring causal label
            label = result["labels"][0]
            score = result["scores"][0]

            entailment_confirmed = (
                label == positive_label and score >= confidence_threshold
            )

            validation_result = {
                "keyword": keyword,
                "sentiment": sentiment_label,
                "hypothesis": positive_label,
                "label": label,
                "confidence": float(score),
                "causal_confirmed": entailment_confirmed,
                "interpretation": f"'{keyword}' {'is a likely cause of' if entailment_confirmed else 'is not a likely cause of'} "
    ...
    def validate_cluster_causal_links(
        self,
        cluster_id: int,
        ...
        for keyword, keyword_score in cluster_vocabulary[:top_n]:
            validation = self.validate_causal_linkage(
                keyword, sentiment_label=sentiment
            )
            if validation:
    ...
    def compute_cluster_similarities(
        self, cluster_lda_features: Dict[int, np.ndarray]
    ) -> None:
        """
    ...
    def get_most_similar_clusters(
        self, cluster_id: int, top_n: int = 3
    ...
    def predict_cascade_effects(
        ...
        source_sentiment = cluster_sentiments.get(cluster_id, {}).get(
        ...
        for similar_cid, similarity in similar_clusters:
            ...
            target_sentiment = cluster_sentiments.get(similar_cid, {}).get(
                "dominant_sentiment", "UNKNOWN"
            )

            # Predict improvement
            improvement_likelihood = (
                similarity  # Higher similarity = higher cascade likelihood
            )
    ...
    def generate_if_then_statements(
        self,
        cluster_id: int,
        cluster_vocabulary: List[Tuple[str, float]],
        cluster_sentiments: Dict[int, Dict],
        ...
        if not cascades:
            ...
            for keyword in top_keywords:
                stmt = (
                    f"If '{self._clean_phrase(keyword)}' issues in Cluster {cluster_id} are addressed, "
                    f"service experience should improve."
                )
            ...
            for keyword in top_keywords:
                cascade_targets = [
                    f"Cluster {p['target_cluster']}" for p in cascades[:2]
                ]
                cascade_text = " and ".join(cascade_targets)

                stmt = (
                    f"If '{self._clean_phrase(keyword)}' issues in Cluster {cluster_id} are resolved, "
                    f"service experience should improve, and {cascade_text} may also benefit."
                )
                statements.append(stmt)
    ...
    def reason(
        self,
        cluster_lda_features: Dict[int, np.ndarray],
        ...
        self.cluster_vocabularies = cluster_vocabularies
        self.cluster_sentiment_results = cluster_sentiments
    ...
    def export_to_dataframe(self) -> pd.DataFrame:
        ...
        for cluster_id, validations in self.causal_validations.items():
            ...
            # Get causal keywords
            causal_keywords = [
                v["keyword"] for v in validations if v.get("causal_confirmed")
            ]
            causal_text = (
                ", ".join(causal_keywords) if causal_keywords else "None confirmed"
            )
    ...
    def get_cluster_summary(self, cluster_id: int) -> str:
        """
        Generate a human-readable causal summary for one cluster.
        ...
        """
        sentiment = self.cluster_sentiment_results.get(cluster_id, {})
        validations = self.causal_validations.get(cluster_id, [])
        cascades = self.cascade_predictions.get(cluster_id, [])
        similar_clusters = self.get_most_similar_clusters(cluster_id, top_n=3)
        if_then_statements = self.generate_if_then_statements(
            cluster_id,
            self.cluster_vocabularies.get(cluster_id, []),
            self.cluster_sentiment_results,
        )
        themes = self._get_theme_phrases(cluster_id, top_n=5)

        total_validations = len(validations)
        confirmed = [item for item in validations if item.get("causal_confirmed")]
        confirmation_rate = (
            (len(confirmed) / total_validations * 100) if total_validations else 0.0
        )

        summary = f"""
╔══════════════════════════════════════════════════════════════╗
║                 Cluster {cluster_id} M3 Summary                ║
╚══════════════════════════════════════════════════════════════╝

🧭 OPERATIONAL FEEDBACK REVIEW
  Feedback Tone: {sentiment.get('dominant_sentiment', 'UNKNOWN')}
  Tone Concentration: {sentiment.get('sentiment_density', 0.0):.3f}
  Evidence Items Reviewed: {total_validations}
  Issue Signals Supported: {len(confirmed)} ({confirmation_rate:.1f}%)
"""
        ...
        summary += (
            "\n  What this means: these comments point to repeated friction in the themes above, "
            "so they are useful as a checklist for process or service review.\n"
        )

        if confirmed:
            summary += "\n  Supported Issue Signals:\n"
            for item in confirmed[:5]:
                summary += f"    - {self._clean_phrase(item['keyword'])} ({item['confidence']:.1%})\n"

        summary += "\n🌉 RELATED FEEDBACK PATTERNS\n"
        if similar_clusters:
            for similar_cluster_id, similarity in similar_clusters:
                summary += (
                    f"  Cluster {similar_cluster_id}: {similarity:.1%} shared pattern\n"
                )
        else:
            summary += "  No similarity data available yet.\n"

        summary += "\n⚡ WHERE A FIX MAY HELP OTHERS\n"
        if cascades:
            for cascade in cascades[:5]:
                summary += (
                    f"  Cluster {cascade['target_cluster']}: "
                    f"{cascade['cascade_likelihood']:.0%} chance of similar improvement\n"
                )
        else:
            summary += "  No significant cascades detected.\n"

        summary += "\n🛠️ SUGGESTED MANAGEMENT ACTIONS\n"
        if if_then_statements:
            for statement in if_then_statements[:3]:
                summary += f"  - {statement}\n"
        else:
            summary += "  No action statements generated yet.\n"
    ...
    def get_summary(self) -> str:
        ...
        summary = f"""
╔══════════════════════════════════════════════════════════════╗
║          Operational Feedback Summary (M3)                  ║
╚══════════════════════════════════════════════════════════════╝

🧭 OVERALL FEEDBACK REVIEW
  Total Clusters Reviewed: {len(self.causal_validations)}
"""

        total_validations = sum(len(v) for v in self.causal_validations.values())
        total_causal = sum(
            len([x for x in v if x.get("causal_confirmed")])
            for v in self.causal_validations.values()
        )
        causal_rate = (
            (total_causal / total_validations * 100) if total_validations else 0.0
        )

        summary += f"  Evidence Items Reviewed: {total_validations}\n"
        summary += f"  Issue Signals Supported: {total_causal} ({causal_rate:.1f}%)\n"
        ...
        for cluster_id, cascades in self.cascade_predictions.items():
            if cascades:
                ...
                summary += f"{top_cascade['cascade_likelihood']:.0%} chance of shared improvement\n"

        summary += f"\n⚡ HIGHEST-IMPACT FOLLOW-UP AREAS\n"
        ...
        for cascade in all_cascades[:5]:
            summary += (
                f"  Cluster {cascade['source_cluster']} → {cascade['target_cluster']}: "
            )
            summary += (
                f"{cascade['cascade_likelihood']:.0%} likely shared improvement\n"
            )
